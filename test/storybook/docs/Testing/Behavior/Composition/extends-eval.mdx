...
import errorImg from './extends-compiler-error.png';

<Meta title="Testing/Behavior/Composition/Extends Evaluation" tags={['dev-only', 'pending-review']} />

# Extends-Only Phase 1 Evaluation

> Historical background: see [Historical @Controller Context](?path=/docs/testing-behavior-composition-historical-controller-context--docs).

## References
- PR enabling component inheritance: <a href="https://github.com/stenciljs/core/pull/6362" target="_blank" rel="noopener noreferrer">PR #6362</a>
- Released in Stencil v4.37.0: <a href="https://github.com/stenciljs/core/releases/tag/v4.37.0" target="_blank" rel="noopener noreferrer">Stencil v4.37.0 Release</a>

## Minimum Required Version
- Stencil >= v4.37.0 for `extends` support in components and mixin composition.

## Goal
For Phase 1 we evaluate whether plain class inheritance (extends) is sufficient for the immediate needs, without introducing a `@Controller` decorator or runtime host APIs.
...
<div style={{ maxWidth: '100%', width: '100%', background: '#fff' }}>
  <table style={{ width: '100%', borderCollapse: 'collapse', tableLayout: 'fixed' }}>
    ...
    <tbody>
      <tr>
        <td>Inheritance</td>
        <td>Shared service base reused across multiple components</td>
        <td><code>service-inheritance/cmp.test.tsx</code></td>
        <td style={{ textAlign: 'center' }}>
          <span style={{ background: '#16a34a', color: '#fff', padding: '2px 8px', borderRadius: 12 }}>PASS</span>
        </td>
      </tr>
      <tr>
        <td>Inheritance</td>
        <td>Lifecycle tapping via base hooks (will/did load/update)</td>
        <td><code>service-inheritance/cmp.test.tsx</code></td>
        <td style={{ textAlign: 'center' }}>
          <span style={{ background: '#16a34a', color: '#fff', padding: '2px 8px', borderRadius: 12 }}>PASS</span>
        </td>
      ...
      <tr>
        <td>Inheritance</td>
        <td>Runtime host state updates from base service (mouse controller example)</td>
        <td><code>service-inheritance/cmp.test.tsx</code></td>
        <td style={{ textAlign: 'center' }}>
          <span style={{ background: '#16a34a', color: '#fff', padding: '2px 8px', borderRadius: 12 }}>PASS</span>
        </td>
      ...
      <tr>
        <td>Mixins</td>
        <td>Compose two behaviors into one component; both lifecycles/methods present</td>
        <td><code>service-mixins/*.test.tsx</code></td>
        <td style={{ textAlign: 'center' }}>
          <span style={{ background: '#f59e0b', color: '#111827', padding: '2px 8px', borderRadius: 12 }}>PENDING</span>
        </td>
      </tr>
      <tr>
        <td>Mixins</td>
        <td>Selective composition (single behavior component) and reuse across components</td>
        <td><code>service-mixins/*.test.tsx</code></td>
        <td style={{ textAlign: 'center' }}>
          <span style={{ background: '#f59e0b', color: '#111827', padding: '2px 8px', borderRadius: 12 }}>PENDING</span>
        </td>
      ...
      <tr>
        <td>Mixins</td>
        <td>Conflict scenario: same-name methods across behaviors and resolution strategy</td>
        <td><code>service-mixins/*.test.tsx</code></td>
        <td style={{ textAlign: 'center' }}>
          <span style={{ background: '#f59e0b', color: '#111827', padding: '2px 8px', borderRadius: 12 }}>PENDING</span>
        </td>
      ...
      <tr>
        <td>Mixins</td>
        <td>Override precedence in component with super() chaining</td>
        <td><code>service-mixins/*.test.tsx</code></td>
        <td style={{ textAlign: 'center' }}>
          <span style={{ background: '#f59e0b', color: '#111827', padding: '2px 8px', borderRadius: 12 }}>PENDING</span>
        </td>
      ...
      <tr>
        <td>Mixins</td>
        <td>Watchers/Props/State merge assertions across composed behaviors</td>
        <td><code>service-mixins/*.test.tsx</code></td>
        <td style={{ textAlign: 'center' }}>
          <span style={{ background: '#f59e0b', color: '#111827', padding: '2px 8px', borderRadius: 12 }}>PENDING</span>
        </td>
      ...
      <tr>
        <td>Mixins</td>
        <td>Lifecycle ordering across multiple behaviors</td>
        <td><code>service-mixins/*.test.tsx</code></td>
        <td style={{ textAlign: 'center' }}>
          <span style={{ background: '#f59e0b', color: '#111827', padding: '2px 8px', borderRadius: 12 }}>PENDING</span>
        </td>
...
<small>
Legend: <span style={{ background: '#16a34a', color: '#fff', padding: '1px 6px', borderRadius: 10 }}>PASS</span> = implemented and green in WDIO; <span style={{ background: '#f59e0b', color: '#111827', padding: '1px 6px', borderRadius: 10 }}>PENDING</span> = planned, not yet implemented.
</small>
...
## Parity with Lit Reactive Controllers (Test Plan)

Reference: <a href="https://lit.dev/docs/composition/controllers/" target="_blank" rel="noopener noreferrer">Lit: Reactive Controllers</a>
...
<div style={{ maxWidth: '100%', width: '100%', background: '#fff' }}>
  <table style={{ width: '100%', borderCollapse: 'collapse', tableLayout: 'fixed' }}>
    ...
    <thead>
      <tr>
        <th>Lit Feature</th>
        <th>Stencil Mapping (extends/mixins)</th>
        <th>Spec Path</th>
        <th style={{ textAlign: 'center' }}>Status</th>
      </tr>
    </thead>
    <tbody>
      <tr>
        <td>External input: mousemove controller</td>
        <td>Base service attaches window listener in <code>connectedCallback</code>, updates host <code>@State</code></td>
        <td><code>service-inheritance/cmp.test.tsx</code></td>
        <td style={{ textAlign: 'center' }}><span style={{ background: '#16a34a', color: '#fff', padding: '2px 8px', borderRadius: 12 }}>PASS</span></td>
      </tr>
      <tr>
        <td>Lifecycle: setup/cleanup</td>
        <td><code>connectedCallback</code>/<code>disconnectedCallback</code> establish and tear down listeners/observers</td>
        <td><code>service-inheritance/cmp.test.tsx</code></td>
        <td style={{ textAlign: 'center' }}><span style={{ background: '#16a34a', color: '#fff', padding: '2px 8px', borderRadius: 12 }}>PASS</span></td>
      </tr>
      <tr>
        <td>Lifecycle: pre/post update hooks</td>
        <td>Map Lit <code>hostUpdate/hostUpdated</code> to Stencil <code>componentWillRender/componentDidRender</code>; assert ordering and DOM reads</td>
        <td><code>service-mixins/*.test.tsx</code></td>
        <td style={{ textAlign: 'center' }}><span style={{ background: '#f59e0b', color: '#111827', padding: '2px 8px', borderRadius: 12 }}>PENDING</span></td>
      </tr>
      <tr>
        <td>Update scheduling (requestUpdate)</td>
        <td>Trigger host re-render via <code>@State</code> mutations from base/mixin; coalescing by counting <code>componentWillRender</code> calls</td>
        <td><code>service-mixins/*.test.tsx</code></td>
        <td style={{ textAlign: 'center' }}><span style={{ background: '#f59e0b', color: '#111827', padding: '2px 8px', borderRadius: 12 }}>PENDING</span></td>
      </tr>
      <tr>
        <td>Multiple controllers per host</td>
        <td>Compose multiple behaviors via mixins; both lifecycles and methods present</td>
        <td><code>service-mixins/*.test.tsx</code></td>
        <td style={{ textAlign: 'center' }}><span style={{ background: '#f59e0b', color: '#111827', padding: '2px 8px', borderRadius: 12 }}>PENDING</span></td>
      </tr>
      <tr>
        <td>Controller composition</td>
        <td>Behavior A internally composes Behavior B (mixin chain); verify call-through</td>
        <td><code>service-mixins/*.test.tsx</code></td>
        <td style={{ textAlign: 'center' }}><span style={{ background: '#f59e0b', color: '#111827', padding: '2px 8px', borderRadius: 12 }}>PENDING</span></td>
      </tr>
      <tr>
        <td>Dynamic attach/detach</td>
        <td>Lit <code>addController/removeController</code> dynamic behavior. Stencil: not supported in Phase 1 (static class composition).</td>
        <td>—</td>
        <td style={{ textAlign: 'center' }}><span style={{ background: '#ef4444', color: '#fff', padding: '2px 8px', borderRadius: 12 }}>GAP</span></td>
      </tr>
      <tr>
        <td>Keyboard input</td>
        <td>Base/mixin listens to <code>keydown</code> on window; updates host state</td>
        <td><code>service-mixins/*.test.tsx</code></td>
        <td style={{ textAlign: 'center' }}><span style={{ background: '#f59e0b', color: '#111827', padding: '2px 8px', borderRadius: 12 }}>PENDING</span></td>
      </tr>
      <tr>
        <td>Resize observer</td>
        <td>Behavior owns a ResizeObserver; exposes attach API; asserts size state updates</td>
        <td><code>service-mixins/*.test.tsx</code></td>
        <td style={{ textAlign: 'center' }}><span style={{ background: '#f59e0b', color: '#111827', padding: '2px 8px', borderRadius: 12 }}>PENDING</span></td>
      </tr>
      <tr>
        <td>Mutation observer</td>
        <td>Behavior observes child changes; state reflects mutation count</td>
        <td><code>service-mixins/*.test.tsx</code></td>
        <td style={{ textAlign: 'center' }}><span style={{ background: '#f59e0b', color: '#111827', padding: '2px 8px', borderRadius: 12 }}>PENDING</span></td>
      </tr>
      <tr>
        <td>Async task lifecycle</td>
        <td>Task-like behavior with states: <code>idle</code> → <code>pending</code> → <code>resolved/error</code>; triggers updates; abort on disconnect</td>
        <td><code>service-mixins/*.test.tsx</code></td>
        <td style={{ textAlign: 'center' }}><span style={{ background: '#f59e0b', color: '#111827', padding: '2px 8px', borderRadius: 12 }}>PENDING</span></td>
      </tr>
      <tr>
        <td>Cleanup on disconnect</td>
        <td>After host removal, external events no longer mutate state; no stray timers</td>
        <td><code>service-mixins/*.test.tsx</code></td>
        <td style={{ textAlign: 'center' }}><span style={{ background: '#f59e0b', color: '#111827', padding: '2px 8px', borderRadius: 12 }}>PENDING</span></td>
      </tr>
      <tr>
        <td>Update completion signal</td>
        <td>Lit <code>updateComplete</code> Promise. Stencil: emulate via <code>componentDidUpdate</code> markers/next frame waits</td>
        <td><code>service-mixins/*.test.tsx</code></td>
        <td style={{ textAlign: 'center' }}><span style={{ background: '#f59e0b', color: '#111827', padding: '2px 8px', borderRadius: 12 }}>PENDING</span></td>
      </tr>
      <tr>
        <td>Directive integration</td>
        <td>Lit directives + controllers. Stencil: N/A; pattern can be simulated with methods returning callbacks for refs.</td>
        <td>—</td>
        <td style={{ textAlign: 'center' }}><span style={{ background: '#9ca3af', color: '#111827', padding: '2px 8px', borderRadius: 12 }}>N/A</span></td>
      </tr>
    </tbody>
  </table>
</div>
...
<small>
Notes: Controllers in Lit are instance-based and can be dynamically added/removed. Stencil Phase 1 focuses on static composition (extends/mixins). Where direct parity isn't possible, gaps are recorded explicitly.
</small>
...
<img src={errorImg} alt="Compiler error when extending from a base class on older Stencil" style={{maxWidth: '780px', border: '1px solid #eee', borderRadius: '4px'}} />
