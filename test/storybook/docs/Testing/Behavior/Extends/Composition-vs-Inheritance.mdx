...
Starting with **Stencil v4.37.0** (via [PR #6362](https://github.com/stenciljs/core/pull/6362)), Stencil components gained support for class inheritance (`extends`) and mixin composition (`Mixin()`). Mixins allow for multiple inheritance, enabling components to inherit from multiple base classes simultaneously. This foundational capability enables a powerful architectural pattern: **moving logic out of components into reusable helper classes, commonly called controllers**.
