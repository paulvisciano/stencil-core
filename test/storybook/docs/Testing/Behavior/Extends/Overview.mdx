...
export const ExtendsTestCasesTable = () => {
  ...
  const allRows = [
    ...
    },
    {
      ...
      types: (
        <>
          <strong>requestUpdate Pattern:</strong> Base class calls this.requestUpdate() - simulates Lit's this.host.requestUpdate()<br/>
          <strong>Component-Owned State:</strong> @State lives on component, controller requests updates explicitly<br/>
          <strong>Abstract Method:</strong> Component implements requestUpdate() - decides how to handle update requests<br/>
          <small>Explicit update requests like <a href="https://lit.dev/docs/composition/controllers/" target="_blank">Lit's ClockController</a></small>
        </>
      ),
  ...
  return <TestCasesTable rows={allRows} previewCount={7} />;
};
...
3. ✅ **Test Case #3 Complete**: Controller-initiated updates (requestUpdate pattern) fully implemented (5 passing tests)
4. **Enhance Existing Test Cases**: Build upon the existing smoke tests to add comprehensive coverage:
