  ...
  const allRows = [
    {
      ...
      desc: 'Basic inheritance - component extends base class with lifecycle hooks and shared methods.',
      types: (
        <>
          Direct inheritance patterns: lifecycle + methods + state<br/>
          <small>Components: {basicInheritanceCount} | Covered: {inheritance.tested}/{inheritance.total}</small>
        </>
      ),
