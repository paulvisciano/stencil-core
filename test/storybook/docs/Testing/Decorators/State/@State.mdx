...
{(() => {
  ...
  // Derive dynamic test counts per case (titles may map multiple tests to one case)
  const uniqueTitles = (arr) => Array.from(new Set(arr.flatMap(i => i.testedBy || [])));
  const primitiveTestsCount = uniqueTitles(groupIn(primitiveSet)).length;
  const complexTestsCount = uniqueTitles(groupIn(complexSet)).length;
