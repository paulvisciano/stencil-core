...
### Phase 1 (Generation-first docs)
- Document all properties of each decorator.
- Define a permutation matrix in `rules.json` for the decorator.
- Generate components from the matrix via script; organize by mode under `matrix/<mode>/`.
...
> Compiling the full set of generated components is the signal in Phase 1: it reveals the true compiler‑enforced rules and gives confidence the compiler can build every permutation. Success criterion: 100% compile. Tests land in Phase 2.
