...
const DECORATOR = getArg('decorator', 'component'); // 'component' | 'state' | 'prop' | 'event' | 'listen' | 'method'
...
const CONFIG = (() => {
  ...
  if (DECORATOR === 'method') {
    const RULES_PATH = path.resolve(__dirname, 'Method/rules.json');
    const DATA_PATH = path.resolve(__dirname, 'Method/coverage-data.json');
    const COMPONENT_DIR = path.resolve(__dirname, '../../../../../test/wdio/method/matrix');
    const TEST_DIR = path.resolve(__dirname, '../../../../../test/wdio/method/tests');
    const OVERLAY_OUT = path.resolve(__dirname, 'Method/coverage-overlay.json');
    return { RULES_PATH, DATA_PATH, COMPONENT_DIR, TEST_DIR, OVERLAY_OUT };
  }
  // default: component
...
// Build listen coverage overlay JSON by parsing WDIO test usage
function buildListenOverlay() {
  if (DECORATOR !== 'listen') return;
  const data = readJson(CONFIG.DATA_PATH);
  let testSources = '';
  try {
    if (fs.existsSync(CONFIG.TEST_DIR)) {
  ...
  const rules = readJson(CONFIG.RULES_PATH);
  const optionOrder = rules.emit?.naming?.includeOptions || ['target','event','capture','passive'];
  ...
  const out = { coverage: readJson(CONFIG.DATA_PATH).coverage, stats, items };
  fs.writeFileSync(CONFIG.OVERLAY_OUT, JSON.stringify(out, null, 2));
  console.log(`Listen coverage overlay written to ${CONFIG.OVERLAY_OUT}`);
}

// Build method coverage overlay JSON by parsing WDIO test usage
function buildMethodOverlay() {
  if (DECORATOR !== 'method') return;
  const data = readJson(CONFIG.DATA_PATH);
  let testSources = '';
  try {
    if (fs.existsSync(CONFIG.TEST_DIR)) {
  ...
  const rules = readJson(CONFIG.RULES_PATH);
  const optionOrder = rules.emit?.naming?.includeOptions || ['returns','async','args'];
  ...
  const out = { coverage: readJson(CONFIG.DATA_PATH).coverage, stats, items };
  fs.writeFileSync(CONFIG.OVERLAY_OUT, JSON.stringify(out, null, 2));
  console.log(`Method coverage overlay written to ${CONFIG.OVERLAY_OUT}`);
}
...
function main() {
  ...
  if (covered !== total) {
    const hint = DECORATOR === 'component' ? 'Component/generate-components' : DECORATOR === 'prop' ? 'Prop/generate-components' : DECORATOR === 'State' ? 'State/generate-components' : DECORATOR === 'Listen' ? 'Listen/generate-components' : 'Method/generate-components';
  ...
  // Build overlay for @Listen after verification
  if (DECORATOR === 'listen') {
    buildListenOverlay();
  }
  // Build overlay for @Method after verification
  if (DECORATOR === 'method') {
    buildMethodOverlay();
  }
