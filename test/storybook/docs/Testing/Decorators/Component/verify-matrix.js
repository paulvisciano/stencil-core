...
const COMPONENT_DIR = path.resolve(__dirname, '../../../../../../test/wdio/component-decorator/matrix');
...
function probeEncapsulationIncrease() {
  ...
  const mutated = {
    ...rules,
    exclusiveGroups: (rules.exclusiveGroups || []).filter(group => group.name !== 'encapsulation'),
  };
  ...
  try {
    ...
    // Keep the temporary rules file for inspection if needed.
  }
...
function buildComponentOverlay({ rules, data, optionOrder }) {
  ...
  console.log(`Component coverage overlay written to ${OVERLAY_OUT}`);
}
...
runVerifier({
  ...
  overlayBuilder: buildComponentOverlay,
  afterVerification: () => {
