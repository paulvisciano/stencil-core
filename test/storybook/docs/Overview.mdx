...
## Learn More
For deeper details on Stencil's architecture, CLI, compiler, runtime, and more, explore the additional documentation files located in the `docs` folder of this repository (`~/docs`).

These Markdown files cover:
...
Open the relevant `.md` files in the `docs` directory to read further about each topic.
