...
<Meta title="Testing/Framework/Context Pack" tags={['dev-only', 'pending-review']} />
...
4) Run coverage from `.ai/testing` to discover gaps:
   - All decorators: `npm run refresh-coverage-matrix`
   - Shortcut (refresh → generate → refresh): `npm run coverage-loop`
   - Specific decorator (examples): `npm run coverage-prop`, `npm run coverage-state`, `npm run coverage-listen`, `npm run coverage-event`
5) Verify matrices (scripted):
   - Per-decorator: `npm run <decorator>:verify-matrix` (enforces parity, grouping, exclusivity; writes overlays)
6) Build and run:
   - Build components: `npm run build-wdio-components`
   - Run tests: `npm run run-component-tests`
   - Targeted run (spec/glob): `npm run run-component-tests:spec -- ./state/**/*.test.tsx`
...
- Verification is scripted: run `npm run <decorator>:verify-matrix` to enforce parity/grouping/exclusivity and write overlays; manual spot checks are optional
...
- [Quickstart: Coverage → Generate → Verify](?path=/docs/testing-framework-quickstart--docs)
- [@Component Strategy](?path=/docs/testing-decorators-component--docs)
- Example Prompts: `.ai/testing/AGENT_PROMPTS.md`
