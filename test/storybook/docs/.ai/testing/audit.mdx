...
- Generator integration
  - `generate-missing-components.js`: derives names from `rules.emit.naming`, groups files via `rules.modes` (shadow/scoped/light), emits exactly one style prop per file from `rules.emit.assets`.
- Verify guard
...
- Scripts/docs
  - `.ai/testing/package.json`: added `verify-matrix` shortcut.
...
- Parity: `.tsx` count under `test/wdio/component-decorator/**` equals covered permutations (192).
...
- Continue generating the next batches by reading `missingPermutations` from `component-coverage-data.json` and creating exactly one component per missing key.
