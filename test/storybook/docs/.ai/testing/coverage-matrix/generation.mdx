...
- Script: `Testing/Decorators/Component/generate-components.js`
...
- `npm run refresh-coverage-matrix` → updates per-decorator coverage data including `Testing/Decorators/Component/coverage-data.json`.
- Shortcut: `npm run coverage-loop` runs steps 1–3 in one go.

2) Generate any missing `@Component` permutations
- `npm run component:generate-components` → creates components under `matrix/` as needed.
