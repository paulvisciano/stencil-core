...
```bash
npm run coverage-component-tests
```
...
- **Build/Test Workflow:** Always follow the documented build and test workflow (`npm run build-component-tests`, then `npm run run-component-tests`). Update documentation if the process changes.
