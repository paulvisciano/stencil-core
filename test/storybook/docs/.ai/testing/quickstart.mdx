...
2. Verify matrix integrity (coverage parity, grouping, exclusivity) BEFORE building (for the active decorator)
   - `npm run component:verify-matrix` (use `npm run state:verify-matrix` for @State)
...
- Verify early:
  - `npm run component:verify-matrix` (or `npm run state:verify-matrix`)
- If the build reports compiler errors after you generate components:
  - Tweak the rules JSON for the active decorator (e.g., `Testing/Decorators/Component/rules.json`) to correct the permutation space or exclusivity
  - Re-run `npm run coverage-loop`
  - Re-run `npm run component:verify-matrix`
