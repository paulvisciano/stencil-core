...
<Meta title="Testing/Decorators/@Component Coverage Strategy" tags={['pending-review']} />
...
- .tsx count under `test/wdio/component-decorator/**` equals the covered count (192)
- All matrix components import `{ Component, h }` from `@stencil/core`
- Invalid permutations pruned: no `shadow: true` + `scoped: true` remain; style prop exclusivity enforced
- Fixed invalid tag file: removed `matrix/component.tsx`; added `matrix/cmp-base.tsx`
- Generator updated/used to emit exactly one component per missing permutation; future gens use `cmp-*` prefix

Next:
- Run coverage + build from `.ai/testing` scripts to validate end-to-end
