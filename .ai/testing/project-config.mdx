...
<Meta title="Testing/Project Config" />
...
- **Component Directory:** `test/wdio` (see `COMPONENTS_DIR` in verify-matrix.js)
- **Coverage Script:** `test/storybook/docs/Testing/Decorators/run-all-coverage.js` (see `COVERAGE_SCRIPT`)
- **Coverage Data:** `test/storybook/docs/Testing/Decorators/coverage-data.json` (see `COVERAGE_DATA`)
...
- **Verification process:** Sampling of the matrix will be done manually. (Refer to `verification.mdx` and `verify-matrix.js` for guidance.)
