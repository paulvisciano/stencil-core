...
<Meta title="Testing/Coverage Matrix/Overview" />
...
The coverage matrix workflow is inherently iterative. Both the generation and verification processes are repeated as new decorator options are added, new components are generated, and coverage gaps are discovered. Each cycle improves the reliability and completeness of the matrix.

- The **generation** process is documented in `generation.mdx`, detailing how to enumerate permutations and generate components/tests.
- The **verification** process is documented in `verification.mdx`, describing how to confirm the matrix's accuracy and ensure all permutations are covered.

Refer to these files for step-by-step instructions and best practices for each phase of the loop.
...
For a high-level overview of how matrix-driven coverage fits into the overall testing strategy, see [Testing Overview](?path=/docs/testing-overview--docs).
